...
from django.contrib.auth.hashers import make_password  # 加密函数，对明文加密
...
class CustomBackend(ModelBackend):
    def authenticate(self, username=None, password=None, **kwargs):
        try:
            user =UserProfile.objects.get(Q(username=username)|Q(email=username))  # Q是并集
...
class ActiveUserView(View):
    def get(self, request, active_code):
        ...
        if all_record:
            ...
            return HttpResponse(u'尚未收到激活码链接')
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            return render(request, 'register.html', {'register_form':register_form, 'msg':'验证码不正确'})
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if login_form.is_valid():
            ...
            password = request.POST.get('password', '')
            user = authenticate(username=user_name, password=password)  # 自动验证
            if user is not None:
                if user.is_active:
                    login(request, user)
                    return HttpResponseRedirect(reverse("index"))
                else:
                    return render(request, 'register.html',{'msg':'用户尚未激活'})
            else:
                return render(request, 'login.html', {"msg": "用户名或者密码错误"})  # 验证用户名和密码的错误
        else:
            return render(request, 'login.html',{'login_form':login_form} )  # 提示login_form的错误
...
class LogoutView(View):
    def get(self, request):
        # django自带的登录退出
        logout(request)
        # reverse django的url反向解析参数为name=,HttpResponseRedirect为重定向
        return HttpResponseRedirect(reverse("index"))
...
class ResetView(View):
    ...
    def get(self, request, reset_code):
        ...
        if all_record:
            ...
            return HttpResponse(u'尚未收到重置密码链接')
...
class UploadImageView(LoginRequireMixin, View):
    ...
    """
    def post(self, request):
        #  第二个参数因为在我们传递文件类型的时候和我传递input的时候的存储位置是不一样的。文件是放在request.FILES下的
        #
